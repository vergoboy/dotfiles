...
PanelWindow {
    ...
    readonly property var defaultSettings: ({
        ...
        "modules":{ "left": ["terminal", "workspaces"],
                    ...
                    "right": ["updates", "battery", "powerprofile", "network", "keyboard", "volume", "toolbox", "logo", "power"] },
    ...
    Component {
        id: cToolbox
        ToolboxModule {
            // Rebuild keyboard navigation when the drawer disappears or
            // reappears (it collapses out of the layout while the tray is empty).
            onCollapsedChanged: Qt.callLater(root.rebuildNavItems)
            // Surface the open tray-menu state up to the window so the pill
            // stays expanded for as long as a tray menu is showing.
            Binding {
                target: root
                property: "trayMenuOpen"
                value: trayMenuOpen
    ...
    Component { id: cKeyboard;    KeyboardModule {} }
